...
{ 
	...
	{ 
		...
		{ 
			...
			double d=VL.Mold();//光传播的距离，等于矢量VL的模
			VL=VL.Unit();//光矢量单位化
			CVector VN=Normal;
			VN=VN.Unit();//法矢量单位化 
			double CosTheta=Dot(VL,VN);
			if (CosTheta>=0)//如果在光源的照射范围内，
			{   
				//第1步,加入漫反射光
	...
	LastC+=Ambient*pMaterial->M_Ambient;//第4步，加入环境光
	LastC.Normalize();//第5步，颜色归一到[0,1]区间
	return LastC;//第6步，返回所计算顶点的光强颜色
}

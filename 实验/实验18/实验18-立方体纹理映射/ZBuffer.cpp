...
void CZBuffer::TextureMap(CDC *pDC,COLORREF **Image)//纹理映射
{
	...
	for(pCurrentB=pHeadB;pCurrentB!=NULL;pCurrentB=pCurrentB->next)
	{
		...
		CP2 ta,tb,tf;//ta和tb代表边上任一点的纹理，tf代表面上任一点的纹理
...
CP2 CZBuffer::Interpolation(double t,double t1,double t2,CP2 p1,CP2 p2)//线性插值
{
	CP2 p;

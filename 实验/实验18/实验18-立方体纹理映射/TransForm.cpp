...
CP3* CTransForm::Translate(double tx,double ty,double tz)//平移变换矩阵
{
	Identity();
	T[3][0]=tx; 
	T[3][1]=ty; 
...
CP3* CTransForm::Scale(double s)//整体比例变换矩阵
{
	Identity();
	T[4][4]=s; 	
	MultiMatrix();
...
CP3* CTransForm::Scale(double sx,double sy,double sz)//比例变换矩阵
{
	Identity();
	T[0][0]=sx; 
	T[1][1]=sy; 
...
CP3* CTransForm::RotateX(double beta)//绕X轴旋转变换矩阵
{
	Identity();
...
CP3* CTransForm::RotateY(double beta)//绕Y轴旋转变换矩阵
{
	Identity();
	double rad=beta*PI/180;
...
CP3* CTransForm::RotateZ(double beta)//绕Z轴旋转变换矩阵
{
	Identity();
	double rad=beta*PI/180;
...
CP3 *CTransForm::ReflectX()//X轴反射变换矩阵
{
	Identity();
	T[1][1]=-1;
	T[2][2]=-1;
...
CP3 *CTransForm::ReflectY()//Y轴反射变换矩阵
{
	Identity();
	T[0][0]=-1;
	T[2][2]=-1;
...
CP3 *CTransForm::ReflectZ()//Z轴反射变换矩阵
{
	Identity();
	T[0][0]=-1;
	T[1][1]=-1;
...
CP3 *CTransForm::ReflectXOY()//XOY面反射变换矩阵
{
	Identity();
	T[2][2]=-1;
	MultiMatrix();
...
CP3 *CTransForm::ReflectYOZ()//YOZ面反射变换矩阵
{
	Identity();
	T[0][0]=-1;
	MultiMatrix();
...
CP3 *CTransForm::ReflectZOX()//ZOX面反射变换矩阵
{
	Identity();
	T[1][1]=-1;
	MultiMatrix();
...
CP3 *CTransForm::ShearX(double d,double g)//X方向错切变换矩阵
{
	Identity();
	T[1][0]=d;
	T[2][0]=g;
...
CP3 *CTransForm::ShearY(double b,double h)//Y方向错切变换矩阵
{
	Identity();
	T[0][1]=b;
	T[2][1]=h;
...
CP3 *CTransForm::ShearZ(double c,double f)//Z方向错切变换矩阵
{
	Identity();
	T[0][2]=c;
	T[1][2]=f;

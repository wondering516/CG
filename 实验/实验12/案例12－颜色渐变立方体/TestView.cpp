...
CTestView::CTestView()
{
	...
	Play=FALSE;
	R=800.0;d=800.0;Phi=90.0;Theta=0.0;
}
...
void CTestView::ReadPoint()//点表
{
	int a=280;//立方体的边长
	//顶点的三维坐标(x,y,z)、颜色c
	P[0].x=-a/2;P[0].y=-a/2;P[0].z=-a/2;P[0].c=CRGB(0.0,0.0,0.0);//黑色
	P[1].x=+a/2;P[1].y=-a/2;P[1].z=-a/2;P[1].c=CRGB(0.0,1.0,0.0);//绿色
	P[2].x=+a/2;P[2].y=+a/2;P[2].z=-a/2;P[2].c=CRGB(0.0,1.0,1.0);//青色
	P[3].x=-a/2;P[3].y=+a/2;P[3].z=-a/2;P[3].c=CRGB(0.0,0.0,1.0);//蓝色
	P[4].x=-a/2;P[4].y=-a/2;P[4].z=+a/2;P[4].c=CRGB(1.0,0.0,0.0);//红色
	P[5].x=+a/2;P[5].y=-a/2;P[5].z=+a/2;P[5].c=CRGB(1.0,1.0,0.0);//黄色
	P[6].x=+a/2;P[6].y=+a/2;P[6].z=+a/2;P[6].c=CRGB(1.0,1.0,1.0);//白色
	P[7].x=-a/2;P[7].y=+a/2;P[7].z=+a/2;P[7].c=CRGB(1.0,0.0,1.0);//品红
}
...
void CTestView::DrawObject(CDC* pDC)//绘制立方体
{
	...
	{		
		CVector VP0P1(P[F[nFace].p[0]],P[F[nFace].p[1]]);//面的边矢量
		CVector VP1P2(P[F[nFace].p[1]],P[F[nFace].p[2]]);//面的边矢量
		CVector VN=VP0P1*VP1P2;//面的法矢量
...
void CTestView::DrawFace(CDC* pDC,int nface)//绘制表面
{
	...
	for(int nEdge=0;nEdge<F[nface].En;nEdge++)//边循环
	{
		...
		Point[nEdge].c=ScreenP.c;
	}
	CFill *fill=new CFill;//动态分配内存 
	fill->SetPoint(Point,4);//初始化Fill对象
	fill->CreateBucket();//建立桶表
	fill->CreateEdge();//建立边表
    fill->Gouraud(pDC);//颜色渐变填充面片
	delete fill;//撤销内存
}
